...
module.exports = { isDirectorySync, renameFileSync, readdirAllSync, createFileSync, getFolderPath, lstatSync, existsSync, createFolderSync, removeFileSync, getImgBase64Sync, openFileExplorerSync, readFileSync, writeFileSync }
...
function createFileSync(filePath, fileName, content = '', cover = false) {
    ...
    const baseFilePath = getFolderPath(filePath)
    /** 不覆盖同名文件 存在同名文件 */
...
function renameFileSync(filePath, fileNameOrNewPath, move = false) {
    ...
    if (move) {
        ...
        fs.renameSync(filePath, path.join(getFolderPath(filePath), isDirectorySync(filePath) ? '../' : '', fileNameOrNewPath))
    }
}
